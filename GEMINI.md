...
| Format/Lint | Project uses standard Maven/Spring Boot conventions. |
...
2. **Test-Driven Development (TDD):** Write failing tests *before* implementation. Aim for >80% coverage.
3. **Non-Interactive Commands:** Use `CI=true` and avoid watch modes or interactive prompts.
4. **Git Protocol:** Attach task summaries to commits using `git notes` as defined in `conductor/workflow.md`.
5. **Multi-Site Isolation:** Always respect `centro_id` for data isolation between different educational centers.
