...
\definecolor{azulMarca}{RGB}{46, 58, 92}
\definecolor{coralMarca}{RGB}{194, 78, 61}
\definecolor{papelMarca}{RGB}{247, 242, 234}
...
\pagestyle{fancy}
\fancyhf{}
\fancyhead[L]{\logomark\enspace\small\textcolor{grisTexto}{\textbf{Schooledule} -- Memoria TFG}}
...
% Marca Schooledule para cabeceras (TikZ, sin dependencia externa)
\newcommand{\logomark}{%
  \begin{tikzpicture}[baseline=-0.08cm]
    \fill[rounded corners=1.5pt, fill=azulMarca] (0,0) rectangle (0.40cm,0.40cm);
    \fill[fill=coralMarca] (0,0) rectangle (0.048cm,0.40cm);
    \node[text=papelMarca, font=\bfseries\itshape\tiny] at (0.225cm,0.20cm) {S};
  \end{tikzpicture}%
}
...
The project consists of the design and development of a comprehensive web application
(educational ERP) for the administrative and academic management of training centres.

The primary objective is to overcome the rigidity of current systems by creating a data
architecture capable of supporting complex scenarios such as multi-site management,
educational curriculum versioning and forensic traceability of grades. The system also
includes a complete administration panel with eight management modules, a granular
grading system per evaluation criterion with recovery support, and a security hardening
plan based on the OWASP Top 10 standard.
...
La gestión académica de los centros de Formación Profesional en España combina
requisitos que ninguna plataforma generalista resuelve de forma satisfactoria:
múltiples sedes físicas bajo una misma razón social, profesores que imparten en
varios centros, evaluación competencial por Resultados de Aprendizaje y Criterios
de Evaluación, y la obligación legal de mantener un expediente académico íntegro
y auditable.
...
Moodle es la plataforma de e-learning de código abierto más extendida en el ámbito
educativo. Sin embargo, su modelo de datos está orientado a la gestión de contenidos
y actividades formativas, no a la administración académica:

\begin{itemize}
  \item \textbf{Sin gestión multi-sede real:} Moodle no tiene un concepto nativo de
        «centro educativo» como unidad de aislamiento de datos. Gestionar varias sedes
        requiere instancias separadas o plugins de terceros con mantenimiento propio.
  \item \textbf{Sin auditoría forense de calificaciones:} El historial de cambios de
        notas en Moodle es accesible para el profesor y puede ser alterado. No existe
        un mecanismo inmutable a nivel de base de datos equivalente a un trigger
        PL/pgSQL append-only.
  \item \textbf{Sin modelo RA/CE:} La evaluación por Resultados de Aprendizaje y
        Criterios de Evaluación ponderados, obligatoria en los módulos de FP según
        el Real Decreto 659/2023, no tiene representación directa en el modelo de
        datos de Moodle.
\end{itemize}

\subsection{Additio App}

Additio es el cuaderno del profesor digital más usado en los centros de FP españoles.
Ofrece una interfaz ágil para la introducción de calificaciones, pero tiene
limitaciones estructurales para el caso de uso planteado:

\begin{itemize}
  \item \textbf{Aplicación de usuario único:} Additio está diseñada para el uso
        individual del docente. No dispone de un panel de administración de centro
        ni de una capa de gestión institucional (matrículas, grupos, imparticiones).
  \item \textbf{Datos en silo:} Cada profesor gestiona su propio cuaderno de forma
        independiente. No existe un expediente académico centralizado consultable
        por el alumno ni exportable por la dirección del centro.
  \item \textbf{Sin API ni integración:} Additio no ofrece integración con sistemas
        de gestión de centros ni posibilidad de automatizar la carga masiva de alumnos
        al inicio de curso.
\end{itemize}

\subsection{ERP educativos propietarios (Raíces, Itaca, Alexia)}

Las plataformas autonómicas como Raíces (Madrid), Itaca (Valencia) o Alexia son
soluciones propietarias gestionadas por las administraciones educativas. Sus
limitaciones para el caso de este proyecto son de naturaleza diferente:

\begin{itemize}
  \item \textbf{Dependencia institucional:} Solo están disponibles para centros
        públicos concertados con la administración correspondiente. Los centros
        privados o de otras comunidades no pueden acceder a ellas.
  \item \textbf{Sin control sobre el modelo de datos:} El equipo técnico del centro
        no puede adaptar la estructura de evaluación, añadir criterios personalizados
        ni integrar el sistema con herramientas propias.
  \item \textbf{Sin despliegue propio:} No es posible instalar estas plataformas
        en infraestructura propia ni auditarlas desde el punto de vista de seguridad.
\end{itemize}

\section{Propuesta Diferencial de Schooledule}

Schooledule nace para cubrir el hueco que dejan las soluciones anteriores, con tres
diferenciadores técnicos verificables:

\begin{enumerate}
  \item \textbf{Multi-sede real con aislamiento garantizado a nivel de base de datos:}
        La columna \texttt{centro\_id} actúa como predicado obligatorio en cada consulta
        académica. El aislamiento no depende de la lógica de la aplicación, sino de
        restricciones en el esquema relacional y de la capa de servicio
        (\texttt{CentroAdminContextService}).

  \item \textbf{Auditoría forense inmutable mediante trigger PL/pgSQL:}
        Cada modificación de calificación activa el trigger
        \texttt{trigger\_auditoria\_notas}, que inserta en \texttt{auditoria\_notas}
        el valor anterior, el nuevo, el email del docente responsable y la fecha exacta.
        Esta tabla no tiene endpoints de escritura desde la aplicación: su
        inmutabilidad está garantizada a nivel de base de datos, no de política de
        acceso.

  \item \textbf{Evaluación competencial alineada con la LOFP:}
        El modelo Resultado de Aprendizaje \(\rightarrow\) Criterio de Evaluación
        \(\rightarrow\) Calificación, con pesos porcentuales configurables sin
        redespliegue, refleja directamente la estructura de evaluación por
        competencias que exige la Ley Orgánica 3/2022 de Ordenación e Integración
...
Permitir la modificación del estado (\texttt{ACTIVA}, \texttt{BAJA}, \texttt{CONVALIDADO}) y del campo \texttt{esRepetidor}. La eliminación solo está disponible cuando la matrícula no tiene calificaciones registradas.
\begin{itemize}
...
\subsubsection{R11F02T04 -- Implementar la asignación de un alumno a todos los módulos de un grupo}
Ofrecer un formulario en \texttt{GET /admin/alumnos/\{alumnoId\}/asignar-grupo} donde el administrador selecciona un grupo y el flag de repetidor. Al confirmar (\texttt{POST /admin/alumnos/\{alumnoId\}/asignar-grupo}), el servicio \texttt{AdminAlumnoService.asignarGrupo()} itera todas las imparticiones del grupo y crea una \texttt{Matricula} en estado \texttt{ACTIVA} para cada una, omitiendo silenciosamente las imparticiones en las que el alumno ya estuviera matriculado. Si el grupo no tiene ninguna impartición, la operación se rechaza con un mensaje explicativo. Por cada matrícula nueva se inicializa la estructura de evaluación mediante \texttt{EvaluacionBootstrapService.bootstrapParaImparticion()}: crea un único \texttt{PeriodoEvaluacion} (nombre \textit{«Evaluación»}, peso 100) y un \texttt{ItemEvaluable} de tipo \texttt{EXAMEN} por cada \texttt{ResultadoAprendizaje} del módulo. El bootstrap es idempotente: si la impartición ya dispone de períodos, no se crea nada.
\begin{itemize}
  \item \textbf{R11F02T04P01 --} Asignar un alumno a un grupo con tres imparticiones, dos de las cuales ya tiene matrícula, y verificar que se generan exactamente una matrícula nueva y que el mensaje de éxito indica el número correcto de matrículas creadas.
  \item \textbf{R11F02T04P02 --} Intentar asignar un alumno a un grupo sin imparticiones y verificar que el sistema rechaza la operación con el mensaje \textit{«El grupo no tiene módulos asignados todavía»}.
\end{itemize}
...
Generar con Apache POI un fichero \texttt{.xlsx} con las columnas: \texttt{username}, \texttt{nombre}, \texttt{apellidos}, \texttt{email}, \texttt{password}, \texttt{centro\_nombre}, \texttt{curso\_academico\_nombre}, \texttt{grupo\_nombre} y \texttt{es\_repetidor}. Devolver el fichero con \texttt{Content-Disposition: attachment}.
\begin{itemize}
  \item \textbf{R18F01T01P01 --} Descargar la plantilla y verificar que el fichero \texttt{.xlsx} contiene exactamente las nueve columnas requeridas en la primera fila y que el resto del fichero está vacío.
\end{itemize}
...
El endpoint recibe el fichero \texttt{.xlsx}, lo parsea con \texttt{UsuarioExcelParserService} y ejecuta las validaciones de \texttt{UsuarioImportValidatorService}. El límite máximo es 200 filas. Si se detecta cualquier error, se lanza \texttt{UsuarioImportException} con la lista de errores por fila y no se persiste ningún registro. Si todas las filas son válidas, para cada fila: (1) si el \texttt{username} ya existe en el sistema, el usuario se reutiliza sin crear ningún registro nuevo; en caso contrario se crea el usuario con rol \texttt{ROLE\_ALUMNO}, contraseña cifrada con BCrypt y el flag \texttt{mustChangePassword=true}; (2) se localiza el grupo por \texttt{centro\_nombre} + \texttt{curso\_academico\_nombre} + \texttt{grupo\_nombre}; (3) se crea una \texttt{Matricula} en estado \texttt{ACTIVA} para cada impartición activa del grupo, omitiendo las imparticiones en las que el alumno ya estuviera matriculado.
\begin{itemize}
...
\begin{itemize}
  \item \textbf{Capa de Presentación:} Veinticuatro controladores Spring MVC anotados
  ...
  \item \textbf{Capa de Lógica de Negocio:} Veintiocho servicios
...
El flag \texttt{mustChangePassword} no solo actúa en el momento del login. El filtro
\texttt{MustChangePasswordFilter} (\texttt{OncePerRequestFilter}, registrado en
\texttt{SecurityConfig} tras \texttt{UsernamePasswordAuthenticationFilter}) comprueba
en cada petición autenticada el atributo de sesión \texttt{mustChangePassword}. Si el
atributo está activo, redirige inmediatamente a \texttt{/change-password},
independientemente de la ruta solicitada. Quedan excluidas de esta redirección las
rutas de gestión de sesión y los recursos estáticos:
\texttt{/change-password}, \texttt{/logout}, \texttt{/login/**}, \texttt{/css/**},
\texttt{/js/**}, \texttt{/images/**} y \texttt{/error}.
...
\begin{enumerate}
  ...
  \item \textbf{Persistencia:} \texttt{UsuarioImportService} resuelve las claves
        foráneas (centro, curso, grupo) contra la base de datos. Para cada fila,
        si el \texttt{username} ya existe en el sistema el usuario se reutiliza
        sin crear ningún registro nuevo (upsert por username); en caso contrario
        se crea el usuario con \texttt{ROLE\_ALUMNO}, contraseña cifrada con
        BCrypt y el flag \texttt{mustChangePassword=true}. A continuación se
        genera una \texttt{Matricula} en estado \texttt{ACTIVA} para cada
        impartición activa del grupo, omitiendo las imparticiones en las que el
        alumno ya estuviera matriculado.
\end{enumerate}

\section{Asignación Directa de Alumnos a Grupos}

Complementario a la importación masiva, \texttt{AdminAlumnoService.asignarGrupo()}
permite matricular a un alumno en todos los módulos de un grupo en una sola operación
desde la interfaz de administración (\texttt{GET/POST /admin/alumnos/\{alumnoId\}/asignar-grupo}).
El servicio itera las imparticiones del grupo seleccionado, crea la \texttt{Matricula}
para cada una que el alumno no tenga ya, y por cada matrícula nueva delega en
\texttt{EvaluacionBootstrapService.bootstrapParaImparticion()}.

\texttt{EvaluacionBootstrapService} crea un \texttt{PeriodoEvaluacion} (nombre
\textit{«Evaluación»}, peso 100) y un \texttt{ItemEvaluable} de tipo \texttt{EXAMEN}
por cada \texttt{ResultadoAprendizaje} del módulo del curso académico en curso.
La operación es idempotente: comprueba mediante
\texttt{periodoRepository.existsByImparticionId()} si la impartición ya dispone de
períodos y, en ese caso, no crea ningún registro adicional. A diferencia del flujo de
importación masiva, \texttt{UsuarioImportService} no invoca a
\texttt{EvaluacionBootstrapService}; las matrículas creadas desde Excel quedan sin
estructura de evaluación hasta que se realice una asignación posterior desde la UI o
se creen los ítems manualmente.
...
La batería de tests unitarios cubre los veintiocho servicios mediante veintiocho
